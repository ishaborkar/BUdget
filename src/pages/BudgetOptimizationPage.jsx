  ...
  // Calculate total current budget
    ...
    {suggestions.length > 0 && (
        <div className="suggestions">
            <h2>Suggestions for Cuts</h2>
            <ul>
                {suggestions.map((suggestion, index) => (
                <li key={index}>{suggestion}</li>
            ))}
            </ul>
            <h3>Total Amount Reduced from Expenses: ${totalCutFromExpenses.toFixed(2)}</h3>
            <h3>Total Amount Reduced from Budgets: ${totalCutFromBudgets.toFixed(2)}</h3>
        </div>
        )}
    </div>
    );
